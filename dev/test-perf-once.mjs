...
const checks = [
  ...
  ['label=pf||pd?(pf&&pd&&pf!==pd?pf+" → "+pd:(pf||pd)):"كل الأيام"', "عمود التاريخ كيعرض الفترة المختارة"],
...
const Q = { n: "مكمل مرض السكري", p: 250, s: "Leader" };
...
/* حالة 2: بلا فيلتر = كل الأيام */
let r2 = wrap(Q, orders, adspend, noCosts, "", "", er, qc);
console.log("── بلا فيلتر (كل الأيام):");
console.log(`  total order: ${r2.total} (المتوقع 4) | Livré: ${r2.liv} (المتوقع 2) | التاريخ: "${r2.row.date}"`);
if (r2.total !== 4 || r2.liv !== 2 || r2.row.date !== "كل الأيام") {
  console.error("❌ حالة كل الأيام ما خدماش مزيان"); bad++;
} else console.log("✅ بلا فيلتر كيجمع كل الأيام — والمنتوج باقي سطر واحد");

/* حالة 3: سطر واحد لكل منتوج — مهما كان عدد الأيام */
const z = [{ n: "مكمل مرض السكري", p: 250, s: "Leader" }, { n: "نظارة القراءة", p: 120, s: "Leader" }];
...
let cat = [{ n: "مكمل مرض السكري", p: 250, s: "Leader" }];
{
  const P = cat.find(Q2 => Q2.n.toLowerCase() === "مكمل مرض السكري" && Q2.s === "Leader");
  cat = P ? cat.map(Q2 => Q2 === P ? { n: Q2.n, p: 260, s: "Leader" } : Q2)
          : [...cat, { n: "مكمل مرض السكري", p: 260, s: "Leader" }];
}
if (cat.length === 1 && cat[0].p === 260) console.log("✅ إضافة نفس المنتوج مرة أخرى: كتحدّث الثمن فقط (بلا سطر جديد)");
else { console.error("❌ الإضافة المتكررة كتزيد سطور!"); bad++; }

if (bad) { console.error(`❌❌❌ ${bad} مشكل`); process.exit(1); }
console.log("✅✅✅ v2.9 خدامة: المنتوج مرة وحدة + الفيلتر كيتحكم فالفترة");
process.exit(0);
